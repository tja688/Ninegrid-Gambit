...
| `Flow/` | Flow 侧遗留/切片；**BoardBriefTipCopy / Session**（#89）；**RoomIconVisualFit**（场地图标落格压缩放）；**ShopBoardSlotResolver**（#92）；**TavernBoardSlotResolver**（#93）；**RewardBoardSlotResolver**（#94）；**InRoomLeaveWatchStructuralTests**（离开监视 Active 顺序 / 扣金 HUD / 选项 Fit / 真卡禁 Fit / SoftBlockOnly） |
