...
| `Explore/` `Attack/` `Pickup/` `UseItem/` `Shuffle/` `Zone/` | 垂直切片（意图 → Command → Director）；补牌统一走盘面稳定化，不再按 Fusion/Drain 分目录 |
...
| `Cheat/` | F12 作弊工具面板契约：**CheatToolCardSearchIndexTests**（候选池仅怪物/机关/道具三类、排除归档卡组、可按卡名/卡组名/技能名/技能描述搜索、选项文案=卡组·卡名）；**CheatToolPanelBootstrapTests**（对齐场景结构：一级六按钮须 `BoxCollider2D`+`CheatToolPanelButton` 而非 uGUI Button；`回复满血选项` / `记录log选项`；加卡二级与 **log 记录二级** WorldSpace Canvas 打开后面板内须补 `GraphicRaycaster`，且初始关闭；关闭二级清空输入 / 还原一级；`TryToggle` 能找到失活的「作弊工具BG」；空物体时仍可最小自举两层；缺 EventSystem 时创建） |
| `Output/` | 描述 / 伤害等输出 |
| `Cards/` | 卡面 Commit、牌库闸、飞行排序、致死表现回归；**CardDisplayModeVisualsTests**（#104：mode 倍率相对预制体基准且可还原）；**DealFlightRemoveCancelContractTests**（同批 Deal→Remove 须取消飞牌，防 choreo 泄漏 / Pickup Buffered）；表现层配置器条目门槛（Skill 不进窗口、HelpCard/Item 保留）+ **空装配清 effectIds / 挂装配投影 / 效果分类标注（道具/遗物/怪物技能/机关技能）/ 同类多载过滤 / skillIds→assemblies 展开**（解耦装配 IA）+ **装配描述自动同步/自定义锁定 / design_text→{param} 参数化**（同键跨装配异值时自动描述提升为 `{装配id.键}` 限定式；装配行「发送自身变量」按钮逐键追加 `{装配id.键}` 标准代码到描述框——缺装配 id 不兜底 templateId，ADR-0035 / #154）等；**ContentArt** 路径约定 / Resources 帧加载 / 断链校验（#66）；**JSON→Catalog 投影**（帮助卡/怪物/技能/遗物/牌组/房间）+ 表 JSON（效果模板 + 装配引用，#67–#70、ADR-0008/0009）；**词条/`{param}`/详情合成**（#71；`{装配id.键}`/`{模板id.键}`/`{卡defId.键}` 限定式插值——前缀命中取值一致才填、异值歧义保留字面量）；**#154 投影缝边界**：缝能填的 `{装配id.键}` 令牌契约校验干净、defId 前缀式令牌缝留字面量且契约报错（`Seam_ExactAssemblyIdToken_FillsAndPassesTokenContract` / `Seam_DefIdPrefixToken_KeepsLiteralAndFailsTokenContract`）；**#155 投影模式选择**：检查模式恒检查描述（局内模板/已提交剩余不泄漏）、实例模式空模板与检查同文、有模板走模板、已提交剩余覆盖初始实参、缺剩余回退初始实参、局内模板令牌契约干净（`Projection_*` 系列，`CardFaceDescriptionProjector`）；**#156 倒计时内容投影端到端**（`CardFaceCountdownProjectionContentTests`：trap.flame 磁盘内容走投影缝——Instance 无剩余显「剩余3次移动后…」、已提交剩余覆盖为「剩余1次…」、Inspect 忽略剩余恒静态「…3次后[death]」）；**#158 首批牌店可升级伤害升格**（`CardFaceDamageParamPromotionContentTests`：help.bomb / help.throwing_knife 生产内容契约——伤害为装配实参 `help.*.use.amount`（kernel 消费）、描述只含 `{装配id.键}`、faceIntro 草稿 ≤26 格、`ValidateCard` 干净）；**#159 道具卡全集逐卡审计**（`CardFaceHelpCardAuditContentTests`：19 张 live HelpCard 生产内容契约——令牌契约干净 / 描述与 faceIntro ≤26 格 / faceIntro 草稿齐全；6 张升格卡描述=限定式且填值=装配实参（恢复10、互换2、盾5、传送1、钱50、绑票1）；无数字散文句不被盲替换改写）；**#160 遗物全集逐卡审计**（`CardFaceRelicAuditContentTests`：51 张 live Relic 生产内容契约——令牌契约干净 / 描述与 faceIntro ≤26 格 / faceIntro 草稿齐全；20 张升格卡填值=装配实参（terror_mask「+1/每击杀6/破坏1只」、血魔「每5点伤害」、肌肉反制「10点MHP/1点伤害」等）；叙事同值双胞胎入库对齐内核常数（threshold/pct/every/slot，`NarrativeArgExpectations`）；无数字散文句不被盲替换改写）；**#161 机关全集逐卡审计**（`CardFaceTrapAuditContentTests`：10 张 live Trap 生产内容契约——令牌契约干净 / 描述、faceIntro、liveTemplate ≤26 格 / faceIntro 草稿齐全；10 张填值=装配实参（捕熊10、刺藤1、滚石格3/格6、烈焰1伤/3次、复活石6次/1张重生骷髅）；叙事同值双胞胎入库对齐内核常数（every/slot，`NarrativeArgExpectations`）；倒计时卡（烈焰/复活石）必作者局内模板；散文句不被盲替换改写）；**右键详述 live 挂载优先**（空白板 JSON + 局内注入技能 / 忽略预设 skillIds）；**Disable Domain Reload 下 Catalog 重绑**；**装配 argsJson 缺占位实参拦截 / 选模板建议实参**；**特效库** `visual_effects.json` 扫描/粘性合并/atlas 帧加载（与 DSL 效果池区分；纯预览阶段） |
| `Flow/` | Flow 侧遗留/切片；**BoardStabilizationSchedulerTests**（Core 每轮补牌须等上一轮 Present ack，稳定态检查不提前解算）；**BoardBriefTipCopy / Session**（#89）；**BoardPlacementStructuralTests**（#104+#105 / ADR-0024：生产全集禁 Fit / 禁 parent 到 GroundAnchors / 落格路径禁写 `localScale` / 无 `slotHitBoxSize`）；**ShopBoardSlotResolver**（#92）；**TavernBoardSlotResolver**（#93）；**RewardBoardSlotResolver**（#94）；**AttributeBoardSlotResolver / AttributePickIndexResolver / AttributeBoardPresenterStructuralTests**（#137：候选格位、视觉候选→当前 Pending 索引、点击须经 RewardChoiceCoreHook / 禁直改 Model / 首选驻留等二次 / Despawn 释放清理）；**InRoomLeaveWatchStructuralTests**（离开监视 Active 顺序 / 扣金 HUD / SoftBlockOnly；选项与真卡均预制体尺度、禁 Fit）；**InRoomItemAcquirePresentationStructuralTests**（ADR-0025：购领须接手 ItemSlots→手牌，禁只碎裂货架）；**RoomOptionFaceVisualsStructuralTests**（商店/卡店特色选项 Instantiate 选项面后须按 defId 应用 JSON 主图标，禁恒显模板默认图标）；**BounceFanChoiceHitTests**（固定 AABB 命中 / 禁 OverlapPoint 回流）；**DirectionalBiasPickerTests**（同层过场四向袋洗牌） |
...
| `PresentationEventMapBeatExhaustivenessTests` | #54/#57/#60/#61/#62：每个 `CoreEventType` 须有表演映射与显式 `PresentationBeat`；`None` 必须带理由；观察型 `BaseStatModified` 不得落在 `Impact`；`CardDealt`/`AvatarAppeared`/`CardFaceChanged` 为 Settled；`DamageDealt`/`EffectTriggered` 为 Impact；`GoldModified`/`RewardOffered` 为 Settled；`ActionCountdownChanged`/`EffectCountdownChanged` 为 Settled（#156） |
...
| `BoardStabilizationContractTests` | **ADR-0034**：Core 以稳定态判断欠补位；一轮 Fill 内触发新空位形成下一独立切片；融合结果在稳定化期间排除并在完成后恢复牌堆顺序；开局 Avatar 回中后再进入开局布局 |
...
| `ItemStatBonusEffectTests` | #97 / #158 / #159：卡店「道具卡数值强化」（`UpgradeItemStats` → `PlayerModel.ItemStatBonus`，每购 +3）——飞刀 6→9、爆弹 4→7（#158：伤害为装配实参 `help.bomb.use.amount`）、耐用盾牌 5→8 与恢复药水 10→13（#159：`help.sturdy_shield.use.amount` / `help.healing_potion.use.amount` 为牌店可升级数值，kernel 经 `EffectValueExpression` 固定量消费）、破击锤 10→13、玩家交战伤害不叠加、火球等派生数值不叠加 |
| `TavernUpgradeCrossNodeEffectTests` | #159 用户流程回归：牌店买两次「道具强化」（+6）→ 离开 → 下一场战斗飞刀打 12；离开牌店后加成仍在 PlayerModel |
...
| `CarryPackPreserveBootstrapContractTests`（Presentation `BattleSession/`） | #107 / #108：`BootstrapRun(preserveRunInventory)` 道具卡格与容量存活；无 preserve 则清空；#159：牌店「道具强化」加成（`ItemStatBonus`）亦须跨 preserve 存活 |
