...
| `Cheat/` | F12 作弊工具面板契约：**CheatToolCardSearchIndexTests**（候选池仅怪物/机关/道具三类、排除归档卡组、可按卡名/卡组名/技能名/技能描述搜索、选项文案=卡组·卡名）；**CheatToolPanelBootstrapTests**（无场景预置时 OpenPanel/TryToggle 须运行时自举全层级：根 Overlay Canvas+GraphicRaycaster+CanvasScaler、缺 EventSystem 时创建 InputSystemUIInputModule、一层五命名按钮须为 uGUI Button（物理命中路径不适用 Overlay UI）、二层含 TMP_InputField+ScrollRect 且打开面板后初始关闭） |
| `Output/` | 描述 / 伤害等输出 |
