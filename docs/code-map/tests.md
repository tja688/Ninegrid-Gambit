...
| `AudioAssetPolicyTests` / `AudioAssetHygieneValidator` | #167：正式音频唯一 `Resources/audio` 根、SFX/BGM 路径与固定 manifest、隔离区排除、导入设置（2D / BGM preload+Streaming / SFX first-use DecompressOnLoad）、重复哈希 / GUID / LFS / Resources 断链与序列化旧根引用检查 |
