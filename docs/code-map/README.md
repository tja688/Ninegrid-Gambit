...
| [ADR-0036](../adr/0036-audio-cue-binding-and-music-ownership.md) | 声音提示与绑定真源；Resources 音频根；MMSoundManager 播放 Adapter；**#170 已落地**唯一期望音乐状态、BGM JSON 解析、Music 轨切歌代数/收口与流程来源诊断 |
...
| `NineGrid.Presentation` | `Assets/Scripts/NineGrid.Presentation/` | 表现层（原 Flow+Cards **合并后的单一程序集**）；#168 `IAudioSystem`/`AudioSystem` 安装在 `PresentationSceneRoot` 主菜单生命周期，业务声音经 `TriggerPulseHub` 转发到 `MMSoundManagerAudioPlaybackAdapter`；音频历史暴露稳定 `BindingKey` 供编辑器 Play Mode 定位 |
| `NineGrid.Presentation.Tests` | `…/Tests/` | EditMode（含 #168 音频 System / cue 扫描 / 播放结构护栏测试；#169 `AudioBindingEditorSessionTests` 覆盖 JSON 工作副本保存/回撤、脏计数、搜索与历史定位） |
