...
| `NineGrid.Content.Editor` | `…/NineGrid.Content.Editor/` | 卡牌表现编辑器（侧栏 **卡面 / 效果池 / 卡组·卡背**：按 `deckId` 分组卡面；`effectAssemblies`/`skillIds` 可换装；效果模板 + 描述词条；Deck 卡背）；`ContentArtBreakLinkValidator` 断链扫描 |
