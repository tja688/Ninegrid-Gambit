...
| `NineGrid.Content.Editor` | `…/NineGrid.Content.Editor/` | 卡牌表现编辑器（侧栏 **卡面 / 效果池 / 卡组·卡背**：按 `deckId` 分组卡面；卡面单一 **效果装配**（统一模板池 + 过渡来源标注；打开怪物时把旧 `skillIds` 展开进 `effectAssemblies`）；效果模板 + 描述词条；Deck 卡背）；`ContentArtBreakLinkValidator` 断链扫描 |
