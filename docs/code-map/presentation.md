...
- **#203 gold-flight 程序化播放器与金币表现时间窗**：`Systems/Vfx/VfxGoldFlightPlayer`（`public`，Pulse only）自治飞币生成、数量分配、散布、错峰、飞行、缩放、排序与软/硬时间窗压缩（`GoldFlightTiming` 纯数学，旧 `GoldGainFxManagerSingleton.ComputeTiming`/`DistributeValues` 等价迁移，末达 ≤ 硬上限 2.2s）；批次创建成功一次性返回 `VfxPresentationPlan`（首达/末达），经 `VfxPlayerStartResult.PresentationPlan` → `VfxCueResult.PresentationPlan` 暴露给 HUD，不建立逐金币回调。终点、排序边界与图标吞噬反馈经专用金币 HUD 域宿主 `Flow/GoldHudDomainHost`（`IGoldHudDomainHost : IVfxDomainHost`）取得：由 `PlayerInfoHudPresenter` 在绑定 HUD 后经显式 `GoldHudDomainHost.Install(playerInfoRoot, goldIcon)` 幂等安装（root/icon 由 Presenter 持有或在其绑定的根下解析，**不再**按名搜场景；`GoldGainPresentationBinder` 不参与安装），播放器只经 `Instance` 使用、不搜索或修改场景；`PunchIcon` 叠加吞噬缩放、窗口后 `SettleIconToBase` 精确恢复图标基础值，`SnapIconToBase` 供扣减/首刷瞬时对齐。`VfxSpatialContext` 新增可选 `Amount`（金币增量，不进绑定主键）供播放器取可视币数；`DefaultVfxPlayerFactory` 接线 `VfxGoldFlightPlayerFactory`。正式声明 `GoldGainVfxCues.FlyIn`（`economy.gold_flight`）+ `vfx_bindings.json` 对应 independent/unscaled 绑定。飞币基础缩放 0.5、排序略高于金币图标。`VfxGoldFlightPlayerContractTests` 覆盖计划有限性、宿主缺失失败、宿主死亡不取消已开始飞币、系统级计划透传与生命周期、图标精确复位。
...
- **#204 删除旧金币 FX 壳**：已删除 `GoldGainFxManagerSingleton` 与场景/Bindings 序列化装配；`PresentationSceneRoot` 不再持有 `goldGainFxManager`。Keypad9 仍由 `GoldGainFxManagerDevKeys`（经 `DevTestSceneInstaller` 运行时装）走 `PresentGainVisual`。新播放器使用 Resources `VFX/GoldFlightCoin`，旧 `Assets/Prefabs/金币预制体.prefab` 已回收。
