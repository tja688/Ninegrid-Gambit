...
- `AudioCueAttribute` + `AudioCueDeclarationScanner` 维护稳定 cue ID、中文音效说明、模块、权威发射者与允许上下文；`GameFlowController.BeginFormalRun` 发射 `ui.main_menu.start`，`EffectTriggerPulseBeatHandler` 以 Core 事件的 `SourceDefId` / `Cause` 发射内容化 `sfx.effect.trigger`。
