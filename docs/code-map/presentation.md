...
- **效果倒计时上卡面（ADR-0035 / #156）** → 效果触发器（`OnSelfMove`/`OnInteract`/`OnCumulative`，DSL `projectKey` 声明投影令牌键 `装配id.键`）推进计数器后由 `CommitEffectCountdownRemainingAction` 广播 `EffectCountdownChanged`（`ResultValue`=剩余、`Message`=令牌键；未触发只推进也广播）→ `PresentationEventMap` Settled → `CardFaceStatHandler` → `CoreCardPresentationMapper.CommitCountdownRemaining` 写快照 `CommittedCountdownRemaining` 并重投影实例表面；检查侧忽略。禁止 View 直读 Core 计数器（ADR-0005 / ADR-0035）
