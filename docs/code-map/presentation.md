...
| `Systems/` | ~19 | `NineGrid.Presentation.Systems` | QF System / 窄能力接口（含 `IAudioSystem` / `AudioSystem` 与 `MMSoundManagerAudioPlaybackAdapter`；音频历史提供稳定 `BindingKey` 供 Editor 定位；**#172 `MusicSystem` 在 Editor/Development Build 审计 MMSoundManager Music 轨，维护 `MusicOverlapAnomaly` / 试听暂停恢复 / 显式 Stop Unknown Music**） |
...
| `Ui/` | 1 | `NineGrid.Presentation.Ui` | #171 运行时玩家音频设置入口：`PlayerAudioSettingsPanel` 在主菜单右下角提供 Master/BGM/SFX 滑条、独立静音与 Reset；仅绑定 `IPlayerAudioSettingsSystem`，与作者调音工作台分离。 |
| `Cheat/` | 4 | `NineGrid.Presentation.Cheat` | **F12 作弊工具面板**（仅 `UNITY_EDITOR / DEVELOPMENT_BUILD`，正式包不含）：接线 MainScene `UI面板/作弊工具BG`（默认失活；SpriteRenderer 世界 UI）。`CheatToolHotkeyHost`（常驻 + F12）、`CheatToolPanelController`（优先按名找场景预置并接线，缺结构才最小兜底）、`CheatToolPanelButton`（`BoxCollider2D` + `PointerHitRouter`）、`CheatToolCardSearchIndex`（怪物/机关/道具三类、排除归档卡组；卡名/卡组名优先表现层 JSON `displayName`）。一级六按钮：一键清关（对齐 QuickTest `-` → `TryForceNodeVictory`）、战斗加卡、金币 +999、回复满血（场景名 `回复满血选项`）、**记录log**（场景名 `记录log选项` → `第二层_log记录面板`）。战斗加卡二级：WorldSpace Canvas 搜索；运行时补 `GraphicRaycaster`；仅战斗阶段；`ShuffleIntoDrawPileAction` 顶插入 + `PresentEventLogSlice`。log 记录二级：Tag 输入 + 保存按钮 → `DiagTraceManualSnapshot.Save`（物理落盘到 Editor `Assets/Notes/Logs/ManualBugSnapshots` / Player 可执行文件旁 `ManualBugSnapshots`，目录与文件名带 `!!!AI-BUG-REPORT!!!` + `!!!AI_READ_THIS_FIRST!!!.md`）。二级关闭清空输入；`PointerHitRouter` 在 EventSystem UI 上时让渡点击 |
...
| `BattleInfoPreview/` | 战斗信息预览：正式 Run 在 `StartBattleNodeAsync` 前硬阻塞；场景 `UI面板/战斗信息展示BG`；图标 idle（非真卡）；`BattleInfoPreviewCopySO`（Resources `Flow/BattleInfoPreviewCopy`）按楼层×关卡配文案；半黑屏 + 详述嵌套退回预览 |
...
- #169/#173/#174/#178 `NineGrid/音频/声音绑定调音工作台`（`NineGrid.Content.Editor`）以 `AudioBindingEditorSession` 持有磁盘快照与工作副本：编辑作者音量、素材内部起播点、绑定延迟、最短播放间隔、启用状态；声明无正式绑定时直接列为 Unbound，Play Mode 历史中的 `Cooldown` / `Unbound` / 播放结果均可按 `BindingKey` 定位。单素材默认不显示池复杂度，启用多素材池后可编辑变体 ID、素材键、权重、音量 trim 与各自起播点；支持单条/全部保存与回撤、普通试听或按真实绑定延迟试听。**保存会把 `authoringStatus` 标为 `humanConfirmed`**。退出 Play Mode、程序集重载或关闭窗口存在脏条目时明确提示；Revert All Dirty 不清播放历史。**#171 Play Mode 增加只读玩家偏好区**：并列每路作者默认与 PlayerPrefs 模拟值，明确不得在工作台编辑或覆盖玩家设置。
...
- **敌方行动阶段** → `RegisterEnemyActionPhase` / `ResolveNextEnemyAction` / `ResolveEnemyActionFinale` 分拍；**ADR-0038 / #184**：仅节奏源=行动的卡在报名链 −1；移动源在换格推进归零后由报名收走；开火窗同拍单向打击 + `OpenCardRhythmFireWindow`（`OnCardRhythmFire` 同步技能）。导演仍由 `EnemyActionPhaseScheduler` 挂攻击/探索剧本末尾（`ActionCountdownChanged` → Settled → `UpdateActionCount`）（ADR-0012 / #81）
- **行动倒计时上卡面** → Core `ActionCountdownChanged`（`ResultValue`=剩余）经 `PresentationEventMap` Settled → `CardFaceStatHandler` Commit `ActionCount`；禁止 View 队列外直读 Counters（ADR-0005 / #81）。`Action_Icon` 节点名候选含「攻击模式」；`Sync_Rhythm_Icon`↔「是否有技能同步触发」；矩阵见 ADR-0038（占位图用模板默认）
...
- **战斗信息预览**：`BattleInfoPreviewPresenter.ShowAndWaitAsync`（`GameFlowOrchestrator.PlayRealBattleAsync` 正式 Run、发牌前）；槽 `BattleInfoPreviewSlotView` 右键 `TryOpenByDefId`；关详述不 Complete 预览
