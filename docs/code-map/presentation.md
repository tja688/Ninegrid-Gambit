...
| `Ui/` | 2 | `NineGrid.Presentation.Ui` | #171 玩家音频设置入口：`PlayerAudioSettingsPanel` 绑定 MainScene `UI面板/局内功能菜单BG`（默认失活）的音量模块；主菜单根级「菜单按钮」打开并 Acquire `半黑屏BG`，点半黑屏 / 关闭面板 / Esc 关闭。仅绑定 `IPlayerAudioSettingsSystem`，与作者调音工作台分离；已移除运行时 ugui 丑按钮。`UiAudioFeedback` 仍服务 uGUI Selectable。 |
| `Cheat/` | 5 | `NineGrid.Presentation.Cheat` | **F12 作弊工具面板**（仅 `UNITY_EDITOR / DEVELOPMENT_BUILD`，正式包不含）：接线 MainScene `UI面板/作弊工具BG`（默认失活；SpriteRenderer 世界 UI）。`CheatToolHotkeyHost`（常驻 + F12 + 无敌模式 Tick）、`CheatToolPanelController`（优先按名找场景预置并接线，缺结构才最小兜底）、`CheatToolGodMode`（无敌模式开关：每 3 秒回满血 / +999 金币 / 攻击 +10；面板关闭后仍由 HotkeyHost 驱动）、`CheatToolPanelButton`（`BoxCollider2D` + `PointerHitRouter`）、`CheatToolCardSearchIndex`（怪物/机关/道具三类、排除归档卡组；卡名/卡组名优先表现层 JSON `displayName`）。一级七按钮：一键清关（对齐 QuickTest `-` → `TryForceNodeVictory`）、战斗加卡、金币 +999、回复满血（`回复满血选项`）、**无敌模式**（`无敌模式选项` → `CheatToolGodMode.Toggle`）、**记录log**（`记录log选项` → `第二层_log记录面板`）。战斗加卡二级：WorldSpace Canvas 搜索；运行时补 `GraphicRaycaster`；仅战斗阶段；`ShuffleIntoDrawPileAction` 顶插入 + `PresentEventLogSlice`。log 记录二级：Tag 输入 + 保存按钮 → `DiagTraceManualSnapshot.Save`（物理落盘到 Editor `Assets/Notes/Logs/ManualBugSnapshots` / Player 可执行文件旁 `ManualBugSnapshots`，目录与文件名带 `!!!AI-BUG-REPORT!!!` + `!!!AI_READ_THIS_FIRST!!!.md`）。二级关闭清空输入；`PointerHitRouter` 在 EventSystem UI 上时让渡点击 |
...
| `BattleInfoPreview/` | 战斗信息预览：正式 Run 在 `StartBattleNodeAsync` 前硬阻塞；场景 `UI面板/战斗信息展示BG` 上挂 `BattleInfoPreviewPresenter`（Inspector 可配）；图标按 **defId 种类去重**；槽位 **固定尺寸 + Cover 铺满 + SpriteMask 裁切**（`BattleInfoSlotArtFit` / `__Art` 子节点），可选 JSON `slotDisplays.battleInfo`（pivot/zoom）覆盖构图，**不复用**卡面 `mainVisual`；`BattleInfoPreviewCopySO`（Resources `Flow/BattleInfoPreviewCopy`）；半黑屏 + 详述嵌套退回预览 |
...
- **战斗信息预览**：`BattleInfoPreviewPresenter.ShowAndWaitAsync`（`GameFlowOrchestrator.PlayRealBattleAsync` 正式 Run、发牌前）；槽 `BattleInfoPreviewSlotView` 右键 `TryOpenByDefId`；关详述不 Complete 预览；图标 Cover 适配见上表 `BattleInfoPreview/`
