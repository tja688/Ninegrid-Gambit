...
- #169/#173/#174/#178/#187 `NineGrid/音频/声音绑定调音工作台`（`NineGrid.Content.Editor`）不再以 UI Toolkit 窗为主界面：菜单启动 `AudioWorkbenchServer` 并打开鉴权 localhost HTML 壳（`实时抓音` / `静态绑定库` / `BGM` 三模式页签；#187 壳层即可，完整抓音交互后续票）。单一 `AudioWorkbenchEditorState` 拥有 SFX/BGM Session、焦点、状态与冲突；Session 暴露工作副本补丁、`BuildWorkingJson`/`LoadFromSnapshots`（补丁只改可编辑字段并夹紧 ≥0；键冲突/空补丁拒绝；cueId/note/module/authoringStatus 仍由 Session 权威）。Play Mode 下每次 SFX 脏改调用 `ApplyWorkbenchCatalog` 且不落盘；唯 `TrySave`/`TrySaveAll` 写盘并标 `humanConfirmed`，保存前跑 Catalog 卫生危象拒绝；save/revert/reload 后立即再热应用。程序集重载用 SessionState（`NineGrid.AudioWorkbench.Transient.v1`）恢复；磁盘相对基线已变则阻塞写操作，仅「恢复临时版覆盖磁盘」与「丢弃临时版用磁盘」。命令面仅约定 workbench commands；浏览器不改 JSON 文本。**#171 玩家偏好仍独立**，工作台不得覆盖 PlayerPrefs。
