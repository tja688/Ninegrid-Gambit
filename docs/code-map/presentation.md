...
- **窗口引导**：`Setup/DisplayModeBootstrap`（2026-08-13 起，Player-only `RuntimeInitializeOnLoadMethod`）：启动强制 1920x1080 窗口化，覆盖 Unity 记忆的旧分辨率/全屏偏好；放大时的整数倍缩放与黑边由 MainScene 主摄像机 PixelPerfectCamera（960x540 参考分辨率，Crop Frame=Windowbox）承担。PlayerSettings 同步改为 1920x1080 / Windowed / 非原生分辨率
