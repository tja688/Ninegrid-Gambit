...
- **排期器** `Flow/Presentation/BattleBeatScheduler`：批次开启装载非 `None` 指令；开批时若上一批仍有未消费指令则 `LogError` 诊断（从未 Flush / Present 的丢弃，非 Settled Unconsumed）；`ReportBeat` 交给第一个 `IBattleBeatHandler.TryApply` 成功者；Settled 后未消费只报不改；支持 `PresentStandalone`（非锁步旁路冲刷，恢复当批 pending；入批前清空以免误报丢弃）；`FlushImpactExcept` 按 Kind 跳过（ADR-0018）；`FlushImpactOnly(kind)` 只冲指定 Kind 并返回派发条数（ADR-0048 两相 Impact 第一相）
...
- **报点（ADR-0048 两相 Impact）**：攻击/反击命中帧 → `FlushImpactExcept(TriggerEffect)`（命中飘字/血甲即时，触发脉冲后置）；用道具 Present：Vacate 前 `FlushImpactExcept(TriggerEffect)`（保飘字坐标）。奖励 Choice：有盘面 Drain 时先 `PresentEventLogSliceExcluding(TriggerEffect)`，Drain 后再 `PresentEventLogSliceOnly(TriggerEffect)`；空 delta 仍整批 `PresentEventLogSlice`。盘面 Drain：**首个 Remove 前** `FlushImpactExcept(TriggerEffect)`（尸体消失前飘字必须可见；无 Remove 时不再尾部兜底冲刷）。禁止在首个 Deal 前抢跑（ADR-0018）。神圣决斗惩罚帧例外：`ReleaseQuarantined + NotifyBeat(Impact)` 全量（刻意对齐决斗 rig）。`PresentStep`：FaceUp 先刷 → 通道（运动全部落地）→ `FlushImpactOnly(TriggerEffect)` 演触发脉冲 → 有脉冲则停 `TriggerCadenceSec=0.35s` → `FlushBeats`（其余 Impact + Settled）→ 等翻牌 Idle → `TryAcknowledge`。批内可见顺序固定：**运动停稳 → 触发脉冲 → 飘字/血甲 → Settled**
