...
| `Ui/` | 7 | `NineGrid.Presentation.Ui` | #171 局内功能菜单入口：`PlayerAudioSettingsPanel` 绑定 MainScene `UI面板/局内功能菜单BG`（默认失活）的音量模块与流程按钮；`AfterSceneLoad` 对失活面板强制 `EnsureBound`（否则不会 Awake，主菜单根级「菜单按钮」挂不上命中代理）。打开：`菜单按钮`，或局内 Esc（`BattleUiDimmerOverlay` 未激活时）；关闭：点半黑屏 / `功能模块/关闭面板` / Esc。`功能模块/回到主菜单` → `ReturnToMainMenuCommand`，`功能模块/退出游戏` → `GameFlowController.QuitGame`。音量仍只绑定 `IPlayerAudioSettingsSystem`，与作者调音工作台分离；已移除运行时 ugui 丑按钮。`UiAudioFeedback` 仍服务 uGUI Selectable。**存档/读档模块**（ADR-0041）：`RunSaveLoadPanel` 由 `PlayerAudioSettingsPanel.EnsureBound` 挂到 `存档/读档模块`（名字含 `/`，须按直接子节点名查找、不可用 `transform.Find` 路径语义）；「保存」「加载」双按钮切模式，条目按场景预置「保存条目模板」「加载条目模板」克隆进「UI槽位」（可视区本地 Y∈[-1.5,+1.5]，行位 1.05/0.30/-0.45/-1.20；读档列表=自动档+3 手动槽）；点击经私有 `RowHitProxy`（Overlay 面 + `CloseHitSort`），保存写 `RunSaveService.SaveCheckpointToSlot`，读取 `RequestLoadSlot` 前先 `CloseIfOpen`。**主菜单 / 人物选择 / 结算三面板（2026-08-12）**：主菜单本体重制于 `Panels/MainPanel`（菜单背景/Logo/副标题/按钮列/角色装饰/版本号；整体走 SortingLayer `UI` 负序 −6..−2，半黑屏 UI/−1 仍可盖住；按钮为 `StartRun`/`TutorialRun`/`SettingsScreen`/`QuitGame` 四 Collider + 左下角 `LanguageToggle` 语言切换（ADR-0046），由 `GameFlowController` 轮询命中——`settingsHit`（→ `PlayerAudioSettingsPanel.RequestOpen`）、`tutorialHit`（→ `BeginTutorialRun` 教学开局，ADR-0042）、`languageHit`（→ `ILanguageSettingsSystem.Toggle`，label 走 ui 键 `menu.language`）与菜单按钮悬停缩放；`BeginFormalRun` 在存档无教学完成标记时先派教学局）；**场景静态标签本地化** `SceneTextLocalizer`（`Ui/`，MainScene 根级同名 GO 单点）：显式序列化 TMP 引用 + ui 键数组（15 条：主菜单 6 + 功能菜单 5 + 战斗信息预览分组标签 3 + LanguageToggle label），Awake 捕获场景中文原文作默认值，订阅 `Changed` 即刷，禁 Find；`CharacterSelectPanel` 绑定 `UI面板/人物选择BG`（默认失活）——「开始游戏」→ `RequestOpen()`（缺预置回退直接 `BeginFormalRun`），战士默认选中，两席帧动画黑影未解锁（近黑 tint + 拒选提示），`出发按钮` → `GameFlowController.BeginFormalRun`，Esc/`返回按钮` 关闭，Shell 相位离开 MainMenu 自动收起；`RunSummaryPanel` 绑定 `UI面板/结算面板BG`——`GameFlowOrchestrator.ShowBattleEndAndReturnAsync` 胜负收口经 `TryShowAndWaitAsync(victory, ct)` 展示进度/金币/九宫互动/最终属性/遗物图标墙/种子（在 `ClearRunSession` 前快照 `RunModel`/`PlayerModel`/Avatar 数据，只读不发 Core 指令），玩家点 `返回按钮` 或 Esc 放行 `EnterMainMenuImmediate`（该方法开头 `CloseIfOpen` 兜底强退路径；场景缺预置回退旧 Notice+延时，不阻断流程）；两面板共用 `WorldUiHitButton`（`IPointerHitTarget` 世界按钮：BoxCollider2D + PointerHitRegistry + 悬停缩放）与 `ResourcesSpriteLoop`（Resources 序列帧循环装饰，仅面板用、不进卡面表现管线） |
