...
| `Vfx/` | 轻量程序化装饰（静态门面 + 运行时 Runner 单例，不进 VfxBindingCatalog）：`CardEdgeDustFx`（落地喷发 / 飞牌双尾迹尘雾）；`BoardRangeGlowFx`（**悬停威胁范围荧光**：悬停场地怪物卡时按 `CommittedPresentation.AttackPattern` 点亮邻格——正交 / 斜角 / 全向映射 `GroundSlotTopology.GetNeighbors`；程序化 9-slice 内缘光环贴片 + `TableNine/SlotGlowAdditive` 加色材质，缓入缓出 + 呼吸；Avatar 所在威胁格加亮；背面卡（ADR-0016 双向惰性）与「无」模式不显示；接线 `GroundCardHitProxy.HoverEnter/ExitClaim` + `ReleaseClaim`，Runner 每帧自检 origin 占格 / 主线 busy 即淡出，不占主线、不进命中路由） |
...
- 生产装配由 `TriggerPulseOutputController` 完成：旧 FX 接 `CardEffectTriggerPulseSink`；audio 接 `DebouncingTriggerPulseSink(AudioTriggerPulseSink, 0.05s)`；VFX 接 `VfxTriggerPulseSink`。`PresentationSceneRoot.WireHosts` 在主菜单阶段即完成该装配。
