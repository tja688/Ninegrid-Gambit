...
- **排期器** `Flow/Presentation/BattleBeatScheduler`：批次开启装载非 `None` 指令；开批时若上一批仍有未消费指令则 `LogError` 诊断（从未 Flush / Present 的丢弃，非 Settled Unconsumed）；`ReportBeat` 交给第一个 `IBattleBeatHandler.TryApply` 成功者；Settled 后未消费只报不改；支持 `PresentStandalone`（非锁步旁路冲刷，恢复当批 pending；入批前清空以免误报丢弃）；`FlushImpactExcept` 按 Kind 跳过（ADR-0018）
...
- **ADR**：[ADR-0005](../adr/0005-card-face-beat-commit.md)、[ADR-0007](../adr/0007-unified-presentation-pipeline.md)、[ADR-0018](../adr/0018-trigger-visible-causality.md)、[ADR-0045](../adr/0045-unified-card-face-stat-projection.md)（与 0001/0002/0004 交叉引用）；指针/高回报率见 [ADR-0006](../adr/0006-windows-high-polling-mouse-mitigation.md)
