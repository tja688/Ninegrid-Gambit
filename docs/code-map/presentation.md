...
- **#202 VFX 三模式 HTML 工作台**：`NineGrid/视觉特效/VFX 绑定调试工作台`（`VfxWorkbenchWeb/`：`实时实例` / `静态绑定库` / `持续状态`）经 `VfxWorkbenchLauncher` → `VfxWorkbenchServer`（#195 `EditorWorkbenchTransport` + `VfxWorkbenchHost` → `VfxWorkbenchEditorState`）；与 Audio 独立端口/DTO/命令/页面。`VfxBindingEditorWorkbenchSession` 维护 cue/state 声明行、脏改与磁盘读写；Play Mode 热应用 `IVfxSystem.ApplyWorkbenchCatalog`（失败不替换旧 catalog；已播 Pulse 不强杀；持续槽按最新 Binding 安全重建）。未保存 `enabled=false` 为临时停用，显式保存永久写盘。`GetWorkbenchSnapshot` / `PreviewWorkbenchBinding` / `ClearWorkbenchStateSlot` 供实例流、Binding 聚合钻取、预览与单槽清除；程序化播放器可观察/预览/停用但不伪造内部调参表。SessionState（`NineGrid.VfxWorkbench.Transient.v1`）恢复与 DiskChanged 冲突门禁；Release 不启动工作台。`VfxWorkbenchServerTests` + `VfxSystemBehaviorTests` 工作台契约。
...
- **#200 交付护栏与终验收口**：`VfxStructureGuardTests` 扫描项目自有源码，禁止第二静态 Pulse Hub、业务直调 `IVfxSystem.RequestCue`/`SetSlot`/`IVfxPulsePlayer.StartPulse`、动态 UID Binding Key / 动态 cue 拼接、程序化播放器绕过域宿主改共享视觉所有权、`GoldGainFxManagerSingleton` 复活；`VfxDeliveryHygieneTests` 扩展正式 Catalog 的 player 登记与素材路径卫生；`VfxCrossSystemContractTests` 覆盖 Hub 三通道、`economy.gold_flight` 正式绑定、播放器工厂与飞币素材路径。Audio transport 回归仍由 #195 `EditorWorkbenchTransportTests` 承担。终验记录见 `Assets/Notes/Logs/VfxAudit/vfx-e2e-verification-200.md`（EditMode 契约已自动化；完整工作台预览/金币真实路径人工勾选，不以局部测试冒充终验）。#192 Spec 实现决策均有代码、测试或明确 Out of Scope 对应。
