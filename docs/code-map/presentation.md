...
- **报点**：攻击/反击命中帧 → `Impact`；用道具 Present 在 Vacate 前报 `Impact`；盘面 Drain：全部运动步（及同批前置 Deal）播完后、**首个 Remove 前**冲刷 `Impact`（无 Remove 则 Drain 尾冲刷）。禁止在首个 Deal 前抢跑——同批 `[Deal, Rotate]` 时否则脉冲/扣血会早于旋转（ADR-0018）；`PresentStep`：FaceUp 先刷 → 通道 → `FlushBeats` → 等翻牌 Idle → `TryAcknowledge`
