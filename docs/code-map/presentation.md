...
- **生成绝对值** `CardFaceEventValues.WithFaceAbsolutes`：`CardSpawned` / 带 uid 的 `CardDealt` / `AvatarAppeared` 写入造卡/发牌时攻甲血；攻取 `GetFaceAttack`（有效攻，**怪物另加 `EnemyAttackDelta` 规则**——龙鳞甲全场-1、邻接光环+1 等，与伤害结算 `GetAttackDamage` 同口径）
- **Permanent 有效攻旁路**（ADR-0005 细化，非对账）：Conditional/常驻光环改有效攻时，Core 发 `BaseStatModified(ResultValue=CardFaceEventValues.GetFaceAttack)`——`AddStatModifier`/`CommitPermanentAttackFace` Apply、以及 Swap/Rotate/Remove/Kill/`DeactivateOwnerEffects` 对盘面「带条件的 Permanent Attack」补扫（注册表存在**条件型 `EnemyAttackDelta`**（邻接光环）时怪物卡面一并补扫）；**`ModifyBaseStat` 模板原子（加攻卡/属性房加攻/献身类技能）的 Attack 分支同样提交 `GetFaceAttack`**（与 `AppendPermanentAttackFaceCommit` 同构，禁止发基础值——否则带遗物/光环时显示落后于伤害结算）；含 `EnemyAttackDelta` 规则的遗物在 `GrantRelic`/`DiscardRelic` 时经 `AppendEnemyAttackDeltaFaceCommitsForBoardMonsters` 对场上怪物补扫；玩家下一次对怪的 `DamageMultiplier` 经 `AppendProjectedBattleAttackFaceCommit` 投影攻、Once 消耗后 `AppendPermanentAttackFaceCommit` 回退；表现层不对账、不直读 Core
