...
| `Ui/` | 14 | `NineGrid.Presentation.Ui` | **战斗日志面板 `BattleLogPanel`（2026-08-13）**：绑定 `UI面板/战斗日志BG`（默认失活），根级「战斗日志按钮」开、面板内 `关闭面板 (1)` / Esc 关；Scroll View 的 `Viewport/Content` 下运行时建单个 `TextMeshProUGUI`（`__BattleLogText`，SmileySans）整段渲染 `BattleLogStore` 富文本，房间段标题 + 效果组头 + 缩进明细，打开即滚到底（默认看当前房间，往上翻是全局历史）。**层级定位是场地层中间态**：排序层 `UI/-12`（canvas −11 / 关闭钮 −10）压在半黑屏（`UI/-1`）与功能菜单（`UI/0`）之下——任何正式 UI 打开都盖住它；`BattleUiDimmerOverlay.IsActive` 时它的吞点面与关闭钮 collider 自动让位，所以既不独占半黑屏也不阻塞别的 UI。按钮可点性由挂在按钮上的常驻 `ToggleButtonGate` 判定（主菜单 / 覆层激活 / 面板已开一律禁用 collider）。#171 局内功能菜单入口：`PlayerAudioSettingsPanel` 绑定 MainScene `UI面板/局内功能菜单BG`（默认失活）的音量模块与流程按钮；`AfterSceneLoad` 对失活面板强制 `EnsureBound`（否则不会 Awake，主菜单根级「菜单按钮」挂不上命中代理）。打开：`菜单按钮`，或局内 Esc（`BattleUiDimmerOverlay` 未激活时）；关闭：点半黑屏 / `功能模块/关闭面板` / Esc。`功能模块/回到主菜单`、`功能模块/退出游戏` **先经 `功能模块/提示框`（`UiConfirmPrompt`）二次确认**，同意才发 `ReturnToMainMenuCommand` / `GameFlowController.QuitGame`。音量仍只绑定 `IPlayerAudioSettingsSystem`，与作者调音工作台分离。`UiAudioFeedback` 仍服务 uGUI Selectable。**通用确认提示框 `UiConfirmPrompt`（2026-08-13）**：绑定场景预置「提示框」节点（多文案 TMP 子节点 + 确认 A4/取消 A3 图标钮），`Show(文案名, onConfirm, onCancel)`，打开时全屏模态阻塞（BlockerHitSort=20000），Esc=取消（`EscapeHandledThisFrame` 供宿主让位）；两处实例：功能菜单（覆盖存档/退出/回主菜单）与完结结算（退出/回主菜单）。**存档/读档模块**（ADR-0041，2026-08-13 改版）：`RunSaveLoadPanel` 由 `PlayerAudioSettingsPanel.EnsureBound` 挂到 `存档/读档模块`（名字含 `/`，须按直接子节点名查找、不可用 `transform.Find` 路径语义）；**默认展示保存面板**（保存钮激活/加载钮失活），保存视图恒四行=默认存档（自动档展示、不可手动覆盖）+3 手动槽（空位直接存、已有存档先经提示框确认覆盖），加载视图只列已有存档（自动+手动 ≤4，无档「暂无存档」）；条目按「保存条目模板」「加载条目模板」克隆进「UI槽位」（行位 1.05/0.30/-0.45/-1.20），点击经私有 `RowHitProxy`（Overlay 面 + `CloseHitSort`），保存写 `RunSaveService.SaveCheckpointToSlot`，读取 `RequestLoadSlot` 前先 `CloseIfOpen`。**主菜单（2026-08-13 增继续游戏）**：主菜单本体在 `Panels/MainPanel`（SortingLayer `UI` 负序 −6..−2）；按钮为 `StartRun`/`ContinueGame`/`TutorialRun`/`SettingsScreen`/`QuitGame` 五 Collider + 左下角 `LanguageToggle`（ADR-0046），由 `GameFlowController` 轮询命中（选人界面/纯黑屏/加载弹窗任一激活时整体让位）；`ContinueGame` → `MainMenuLoadPanel.RequestOpen()`（`MainPanel/ContinueGame/加载的UI槽位` 预置弹窗：自动档优先 + 手动槽 ≤4 行、点条目 `RunSaveService.RequestLoadSlot`、X/Esc 关，无档返回 false 播拒绝音+短 Notice）；`BeginFormalRun` 在存档无教学完成标记时先派教学局。**场景静态标签本地化** `SceneTextLocalizer`（MainScene 根级同名 GO 单点，显式序列化 TMP+ui 键、禁 Find）。**选人界面 / 完结结算（2026-08-13 重制，纯黑屏底幕）**：底幕经 `PureBlackScreenOverlay`（`UI面板/纯黑屏BG`，引用计数 + Swallow；绑定时防御性摘除误挂的 `BattleUiDimmerOverlay` 单例组件）；`CharacterSelectPanel` 绑定 `UI面板/选人界面BG`——「开始游戏」→ `RequestOpen()`（缺预置回退直接 `BeginFormalRun`），角色1=战士（`立绘/__Art` 挂 `ResourcesSpriteLoop` 会动 idle + 专属道具卡槽显示初始遗物图标，**点击立绘即选定并出发** `BeginFormalRun`），角色2/3=未解锁席（Layla/Icey 帧动画黑剪影 + 拒选音 + 描述闪橙），难度三档可点选（普通默认、全部路由默认数据，记录进 `RunSetupSelection` 静态存根供结算展示），`回到主菜单 (1)` 直接返回不提示、小字悬停才显示，Esc 关闭，Shell 相位离开 MainMenu 自动收起；`RunSummaryPanel` 绑定 `UI面板/完结结算BG`——`GameFlowOrchestrator.ShowBattleEndAndReturnAsync` 胜负收口经 `TryShowAndWaitAsync(victory, ct)` 展示胜负大字（胜/负文案与配色不同）/会动战士立绘/所选难度图标/遗物墙（12 占位槽按视觉序填充）/文字统计（时长/击杀/损血/道具使用，数据源 `RunRecapTracker`），`回到主菜单`/`退出游戏` 先经提示框确认，`再来一局` 放行收口后等 Shell 回 MainMenu 再自动 `CharacterSelectPanel.RequestOpen()` 重开选人（只读不发 Core 指令；场景缺预置回退旧 Notice+延时）；面板共用 `WorldUiHitButton`（世界按钮：BoxCollider2D + PointerHitRegistry + 悬停缩放 + 悬停进出回调，onClick=null 即纯阻挡层）、`ResourcesSpriteLoop`（Resources 序列帧循环装饰）与 `PanelArtUtility`（`__Art` 装载 + `FitWorldHeight` 按包围盒等比缩放并回中——战士帧 pivot 在脚底，帧高常数 Play 实测校准） |
...
| `Tutorial/` | **教学关卡模块**（[ADR-0042](../adr/0042-tutorial-level-module.md)）：`TutorialRunOptions` / `GameFlowRunOptions.CreateTutorial`（载荷即模式）；`TutorialProgressStore`（完成标记经 `RunSaveStoreHook` 独立槽 `tutorial_profile`）；`TutorialDeckPlan`（四波卡表 + 阅读序→抽牌堆序换算，开局 `PreserveDealOrder` 保序发牌）；`TutorialBattleDirector`（逐帧巡检换波：清场批→补发批→`BoardStabilizationScheduler` 经 `MutateMainline` 挂主线，批次投影**必须进导演私有 `QueuedBoardPresentChannel`**（投生产 Explore 通道无人消费 → 换波零表演）；换波条件满足即挂 Opening 输入门至新波 `ScanWave` 完成才解锁；机关无限补位 / 飞刀练习靶走非锁步顶插）；`TutorialClearWaveCommand` / `TutorialInjectWaveCommand`。编排入口 `GameFlowOrchestrator.RunTutorialAsync`（不进节点循环；通关标记完成后按载荷转正式开局或回主菜单；战败不标记、不触发 `HandleRunEnded`）。主菜单入口：`GameFlowController.BeginFormalRun` 无完成标记时改派教学；`MainPanel/TutorialRun` 按钮（serialized `tutorialHit` + FindDeep 回退）任意时刻重进 |
...
- **怪物遭遇正式配置（#134 起）**：七套正式主题卡组（`ThemeDeckStableMapping` + `ThemeDeckFloorTierMapping`，deckId 为历史不透明主键）在遭遇表 `monster_decks.json` 按难度档进层池（`WeakElite`/`StrongElite`/`Boss` → 1/2/3 层；见 ADR-0022）；**每层开局**由 `RewardSystem.FindMonsterDeck` 从本层池均匀随机绑一套未用过的（候选按 deckId 排序以保证同种子可复现）。**新开局种子**：`BattleSessionExecutor.BootstrapRun` 无显式 `InitialGameOptions` 时走 `InitialGameOptions.CreateForNewRun()`（Guid 派生非 0 种子；编辑器 Play / 正式「开始游戏」/ QuickTest 均如此），不再默认 `Seed=1` 锁死遭遇；跨关 `preserveRunInventory` 沿用本局种子。层数攻血叠加见 `MonsterFloorStatScaling`。过渡卡组 `deck.transition` 与池外流浪卡（`deck.wandering_legion` 等）已**归档**——表行 `Reserve` + 成员 `isReserve`，保留 JSON 但不再被 RewardSystem 序列抽卡选中（仍被效果模板 `ShuffleInto` 按 defId 直生，见 ADR-0029）。卡 JSON 保留 `sequence`（1–5）与 `designSlotName`（近战1/远程2…，仅表现、不进 Core）；编辑器可改所属卡组 / 槽位名 / 序列 / 等级。菜单 `NineGrid/Content/校验怪物遭遇配置（过渡期|交付就绪）` 一键校验；交付就绪为正向门禁（ADR-0029）
...
- **详述图标词条 hover（ADR-0037，2026-08-13 扩到卡面图标）**：`Flow/CardInspectIconHover` 只挂详述预览卡面（战斗内场上卡面不挂），读口走 `WorldPointerUtility`，**不注册 `PointerHitRegistry`**——描述内联 sprite 用 `TMP_TextUtilities.FindIntersectingSprite`，卡面机制图标（攻击 / 护甲 / 血量、倒计时、攻击模式、技能同步触发）用 `Cards/Presentation/CardFaceIconGlossaryTargets` 按节点名取 `SpriteRenderer` + 世界 AABB 最小面积命中且复核 `enabled`；命中写 `CardInspectGlossaryListView` **首行** hover 槽。倒计时与攻击模式的词条键随快照 / `CardPresentationConfigCatalog.rhythmSource` 动态解析，复用 Core `AttackPatternRules.Token*` / `CardRhythmRules.Token*`
