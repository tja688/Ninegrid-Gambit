...
- **命中批盘面 delta** → `FieldBattlePresentationExecutor` 在 lunge 后 `Drain` 同批 OnBattle 步骤（如逃避 `Swap`）。主目标**存活**时正常重放其位移（Vacate 前 Drain，否则 Core 已换位而表现占格仍旧 → OccupancyDesync）；主目标**已死**时改为先 MarkFieldDead + Vacate + 离锚、再 Drain——Core 在击杀落地（KillIfDead FollowUp）前 OnBattle 旋转/换位会真实移动尸体，表现侧不重放尸体位移（`ForHitPresentDrain(stripVictimMotion)` 剥离尸体 Move/Swap；Rotate 步只带方向，旋转「无尸体的环」，终态占格与 Core 击杀后一致，尸体不跟转）。主目标尸体 Remove 照旧剥离，走 Vacate/FinalizeLethal。中间 Drain 的 `drainAfter` 占格断言豁免「待 Vacate 主目标」(`C0/Puid`)；Drain 后 `AssertHitPresentOccupancySync` 做最终断言。Fill/Rotate 仍属击杀后剧本。漏 Drain 或真 desync 仍会 `OccupancyDesyncLatched`
