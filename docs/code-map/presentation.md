...
- **#197 Sprite Sheet 播放器与空间契约**：`Systems/Vfx/` 下 `VfxSpriteSheetPlayerFactory` / `VfxSpriteSheetPulsePlayer` 实现 `sprite-sheet` Pulse：经 `VfxMaterialFrameSource`（只读 visual_effects `sheetPath`，不合并索引 defaultFps/defaultScale）+ Resources `LoadAll<Sprite>` 加载；`VfxSpriteSheetFrameOrder` 保证 `spritesheet_N` 数字序稳定。绑定提供 fps/scale/startOffset；有限循环默认 1 次；时间基默认 scaled（`VfxSpriteSheetPlayback` 支持 unscaled）。`VfxSpriteSheetVisualPool` 默认安全池化并 `ResetVisual`；Editor/Development 记录 create/reuse/release/destroy（`VfxSpriteSheetPoolDiagnostics`）。附着型挂 `IVfxDomainHost` 父级/跟随/遮罩/排序边界，宿主丢失正常结束；独立型在接纳时固定世界位并挂 `VfxIndependentSpatialRoot`，自然播完。播放器不改共享相机、Canvas、卡级 SortingGroup 或 L0–L3 塔。素材库 Editor 预览仍走 `VisualEffectPreviewHost` 索引试听。`VfxSpriteSheetPlayerContractTests` 覆盖帧序、有限循环、时间基、池化 Reset、Attached/Independent 生命周期。
