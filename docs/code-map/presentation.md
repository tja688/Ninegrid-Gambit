...
- **命中批盘面 delta** → `FieldBattlePresentationExecutor` 在 lunge 后、Vacate 前必须 `Drain` 同批 OnBattle 步骤（如逃避 `Swap`）；主目标尸体 Remove 经 `BoardPresentationMerge.ForHitPresentDrain` 剥离，仍走 Vacate/FinalizeLethal。中间 Drain 的 `drainAfter` 占格断言豁免「待 Vacate 主目标」(`C0/Puid`)；主目标 Vacate 后 `AssertHitPresentOccupancySync` 做最终断言。Fill/Rotate 仍属击杀后剧本。漏 Drain 或真 desync 仍会 `OccupancyDesyncLatched`
