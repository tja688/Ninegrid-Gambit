...
> **修复注记（已落地）**：洗入批 Present 结算后 `CardDeckManagerSingleton.SyncVisualOrderFromDrawPile(DrawPileUids)` 强制对齐视觉序；Editor/Dev 顶牌断言抓回归。详见 `docs/code-map/presentation.md`「抽牌堆视觉序不变量」。
...
3. ~~表现层卡组堆需在 Core 重洗后重建/对账视觉顺序（或改为卡背显示，玩家不再预判"下一张"）。~~ **已按 DrawPile 对账修复**（`SyncVisualOrderFromDrawPile`）。
