...
局内卡牌实体收敛为**一套卡牌底盘**（变换塔、SortingGroup、命中代理、效果挂点；由 `老Standard Card` 就地演化并保留场景 GUID）加**四套卡面模板**（玩家 / 怪物 / 道具 / 遗物），按 `CardPresentationKind` 显式表在 Spawn 时挂入 L4。Kind 映射为：Avatar→玩家；Monster→怪物；HelpCard/Item/PlayerCard→道具；Relic→遗物。玩家技能不进卡面/Spawn。
...
「一张 Standard Card 顶所有」无法承载四类对象差异化的卡面维度（行动计数、描述、卡背、世界 TMP 等），而运动/排序/命中仍是跨 Kind 公共黑盒——拆成四套完整运行时 Prefab 会四倍复制底盘并与现有塔/飞行抬序冲突。胖投影 + 编排 Commit 针对长期「数值抢先跳变」：可见值提交点收口到串行表演队列，而非 `ApplyToManagedCard`/`Set*` 旁路。装配槽表把 Catalog 的 icon/face 硬字段升级为可扩展语义槽，并让描述插入与主图标/卡背同一真相源。

## 考虑过的替代

- **四套完整运行时 Prefab**：否决——与「不跟现有底盘冲突」及塔/SG/HitProxy 复用目标相反。
