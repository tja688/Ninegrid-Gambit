...
9. **翻牌时序门控**：`PresentStep` 在 `IPresentChannel.Begin`（hop / 移位等）之前先 `FlushUpdateFaceUp` 并等到 `FlipPlaybackCoordinator.IsIdle`；通道完成、`FlushBeats` 后再等 Idle，才 `TryAcknowledge`。同批多张翻牌全局串行，禁止 Handler 对 `PlayFlipAsync` 直接 `.Forget()`。
