...
## 修订（2026-08-16）：商店货架回归房间.md（#211）

策划权威《房间.md》规定商店固定出售：**1 张宝箱卡 + 3 张随机属性相关道具卡 + 1 张食品卡 + 道具牌格扩容**（未满级时）。

- **恢复渠道**：`BuildShopShelves` 第一格恢复上架 `help.common_chest_card`；属性货架由 1 张扩为 3 张（各独立 40/40/20 加权摇取，可重复）；移除恢复药水货架。
- **仍不进随机注入池**：宝箱卡仍只经定向渠道（含商店固定格、宝箱房、奖励房、击杀掉落、遗物效果），不进 `ItemSourcePoolDefIds` 随机装填。
- **废止**：下方「修订（2026-08-12）」中「商店不再上架宝箱」一条；精英房混合注入池移除宝箱的决策**保留**。
