...
- **插入时机（默认战斗房）**：在 `RewardSystem.BuildNodeDeckOptions` 阶段将离开机关编入敌侧发牌池，经正常开局发牌/补牌上场（与常规机关装填同路径）。不设强制上场或软锁特判。
- **插入时机（层主房例外）**：`RoomKind.Boss`（每层最终节点）仍为：击破本节点**开局编入的层主**（Boss Counter）后才洗入；普通房开局编入规则不生效。局中新生层主不计。
