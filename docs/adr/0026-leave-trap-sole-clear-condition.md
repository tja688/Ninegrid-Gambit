...
- **插入时机（默认战斗房）**：在 `RewardSystem.BuildNodeDeckOptions` 阶段将离开机关编入敌侧发牌池，经正常开局发牌/补牌上场（与常规机关装填同路径）。开局洗牌后**必落在抽牌堆后半段**（下标 ≥ ⌈N/2⌉，0 为下一张；若已在后半段则保持）。不设强制上场或软锁特判。
- **插入时机（层主房例外）**：`RoomKind.Boss`（每层最终节点）仍为：击破本节点**开局编入的层主**（Boss Counter）后才洗入；洗入时**置顶**为下一张补牌。普通房开局编入规则不生效。局中新生层主不计。
...
策划要的是可恋战的出口，而不是清光怪自动收摊。门只吃交战，是为避免 AOE/道具流在恋战时误拆出口；魔免进一步让传送等「效果」对离开机关直接失效（可选中、不改 targeting），避免出口被洗回卡组卡死主线；清关时仅当玩家已清完开局真怪才帮卖场上残留道具，有怪残留就离开则不帮卖，把「要不要带走」仍主要逼回道具卡格与主动回收。
