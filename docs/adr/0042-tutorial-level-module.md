...
局内强制教学关卡不再从主流程进入：`GameFlowController.BeginFormalRun` 一律正式开局；主菜单 `TutorialRun` 失活；`BeginTutorialRun` 空操作。玩家可自行翻阅的教学改挂场景「教程按钮」→ `TutorialKnowledgeBookController`（`UI面板/教学知识库BG`）。本 ADR 描述的导演/五阶段/保序发牌仍是历史实现，代码保留但生产路径不再调用。
