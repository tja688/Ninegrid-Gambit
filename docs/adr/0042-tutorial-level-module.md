...
3. **行动/移动计数**：两张提示卡 + 行动假人 / 移动假人（节奏归零自毁，可硬砸）；双离场 → 阶段4。节奏自毁挂 `tpl.tutorial.dummy.rhythm_death`，须声明 `HasOwnerEntity` + `CardZoneTriggerable`（ADR-0010）；空 `requires` 会在 `SpawnCard` 激活效果时熔断，场上只剩提示卡。
