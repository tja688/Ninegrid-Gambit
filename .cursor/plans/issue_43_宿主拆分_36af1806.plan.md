...
- 将 [BattleSessionController.cs](Assets/Scripts/NineGrid.Presentation/Flow/BattleSessionController.cs) 中 Channel、三个 Intent Factory、`RoutingIntentScriptFactory` 与 Director 的构造迁入 `PresentationCompositionRoot.Install(bindings)`。
...
- 将 [GroundFieldView.cs](Assets/Scripts/NineGrid.Presentation/Cards/GroundFieldView.cs) 收缩为锚点、布局参数、Skeleton presenter 与 Transform 操作的场景 view adapter；不再持有占格表、CTS 或业务门禁。
...
- 将 [FieldBattleView.cs](Assets/Scripts/NineGrid.Presentation/Cards/FieldBattleView.cs) 收缩为 `CardAttackBasicAdapter` 与 `BattleEncounterCatalogSO` 的场景 view；删除 `fieldManager`、运行时 Find 和 `Battle` pass-through 属性。
...
- 从 [GameFlowController.cs](Assets/Scripts/NineGrid.Presentation/Flow/GameFlowController.cs) 删除 `_state`、异步流程、对 InBattle/Selector 的持有和枚举镜像 cast；Notice/UI 由输出 Controller 投影。
...
- 同时移动 `.cs` 与原 `.meta`，保留 GUID：`BattleSessionController`→`BattleSessionController`、`GroundFieldView`→`GroundFieldView`、`FieldBattleView`→`FieldBattleView`、`GameFlowController`→`GameFlowController`；不保留旧类继承别名。
