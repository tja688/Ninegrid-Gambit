...
{
    /// <summary>
    /// 牌堆开局发牌：复数 <see cref="CardDeckSubstituteFlow"/> 飞入，槽间间隔逐张加速。
    /// </summary>
    [DisallowMultipleComponent]
    [MovedFrom(true, "NineGrid.Presentation.Performance", null, "CardDeckDealCardsPerformance")]
    public sealed class CardDeckDealFlow : MonoBehaviour, IDirectedFlow
    {
        [Header("Substitute Unit")]
        [SerializeField] private CardDeckSubstituteFlow substituteFlow;
        ...
        public void PlayPreview()
        {
            ...
            {
                Debug.LogWarning($"[{nameof(CardDeckDealFlow)}] ring slots could not be resolved.", this);
                return;
            }
        ...
        public float ComputeTotalDuration(int cardCount)
        {
            ...
            float scaledMoveDuration = substituteFlow != null
                ? substituteFlow.MoveDuration / speed
                : 0.3f / speed;
            float timeline = 0f;
        ...
        private Sequence BuildSequence(IReadOnlyList<Transform> cards, IReadOnlyList<Transform> slots)
        {
            ...
            substituteFlow.ApplySequenceSettings(sequence);
            ...
            for (var i = 0; i < pairCount; i++)
            {
                ...
                Tween move = substituteFlow.CreateMoveTween(card, slot);
                if (move != null)
                {
        ...
        {
            float substituteSpeed = substituteFlow != null ? substituteFlow.PlaybackSpeed : 1f;
        ...
        private void EnsureSubstituteReference()
        {
            ...
            {
                substituteFlow = GetComponent<CardDeckSubstituteFlow>();
            }

            if (substituteFlow == null)
            {
                substituteFlow = gameObject.AddComponent<CardDeckSubstituteFlow>();
            }
        }
    }
}
