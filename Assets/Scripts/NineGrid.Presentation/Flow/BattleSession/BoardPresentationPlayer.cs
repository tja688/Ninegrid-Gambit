    ...
    internal sealed class BoardPresentationPlayer
    {
            ...
            int rotateScanStart = 0)
        {
            ...
            for (var i = 0; i < deals.Length; i++)
            {
                ...
                // 已在场地但错位：迁回目标格，禁止走「已在场地→发牌失败」留下 sticky 空槽。
                if (fieldManager.TryGetSlotOf(deal.Uid, out var occupiedSlot)
                    && occupiedSlot != deal.Slot
                    && occupiedSlot != visualTargetSlot)
                {
                    ...
                    if (fieldManager.RequestRelocateOccupancy(
                            ...
                            skipBusyGuard: true))
                    {
                        if (fieldManager.TryGetCardAt(deal.Slot, out var relocated)
                            && relocated != null)
                        {
                        ...
                        ChoreoTraceSink.SafeEmitAnomaly(
                            "DealMisplacedRelocate",
                            deal.Uid,
                            "from=" + occupiedSlot + ";to=" + deal.Slot);
                        continue;
                    }
                }
            ...
            CancellationToken ct)
        {
            ...
            if (entry.Kind == ShuffleIntoDeckEventKind.ExistingCard)
            {
                if (cardManager.TryGet(entry.Uid, out var existing) && existing != null)
                {
                    ...
                    {
                        Debug.LogWarning(
                            $"[InBattleManager] ExistingCard 回库发射失败 uid={entry.Uid} mode={existing.DisplayMode}");
                        return;
                    }

                    await deckManager.WaitReturnSettledAsync(entry.Uid, ct);
                }
