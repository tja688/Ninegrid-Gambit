...
namespace NineGrid.Flow
{
    internal sealed partial class BattleSessionExecutor
    {
        ...
        private async UniTask StartWalkSandboxNodeInternalAsync(CancellationToken cancellationToken)
        {
            if (_isBusy)
            {
                Debug.LogWarning("[BattleSession] StartWalkSandbox：检测到粘连忙碌，强制复位后继续。");
                CancelPresentationWork();
                _drainInFlight = false;
                _isBusy = false;
                PresentationInputGates.ForceEndExternalHold("StartWalkSandbox.staleBusy");
            }

            if (Cards == null || Deck == null || Field == null)
            {
                Debug.LogError("[BattleSession] 缺少 Card/Deck/Field 管理器，中止跳格沙盒入场。");
                return;
            }
            ...
            try
            {
                ...
                if (!EnsurePresentationRuntimeInstalled())
                {
                    Debug.LogError("[BattleSession] 表现意图运行时未启动，中止跳格沙盒入场。");
                    return;
                }
                ...
                var phase = arch.GetSystem<IPhaseSystem>();
                if (!phase.CanExecute(GameCommandKind.StartNode)
                    && !phase.CanExecute(GameCommandKind.StartWalkSandboxNode))
                {
                    arch.GetSystem<IPresentationSyncSystem>()?.Clear();
                    PresentationInputGates.ForceEndExternalHold("StartWalkSandbox.unlock");
                    if (!phase.CanExecute(GameCommandKind.StartNode)
                        && !phase.CanExecute(GameCommandKind.StartWalkSandboxNode))
                    {
                        BootstrapRun(preserveRunInventory: true);
                        if (!EnsurePresentationRuntimeInstalled())
                        {
                            Debug.LogError("[BattleSession] Bootstrap 后表现意图运行时未启动，中止跳格沙盒。");
                            return;
                        }
                    }
                }
                ...
                _nodeEventLogStart = arch.GetSystem<IActionPipelineSystem>().EventLog.Entries.Count;
                var result = phase.StartWalkSandboxNode();
                if (!result.Accepted)
                {
                    Debug.LogError($"[BattleSession] StartWalkSandboxNode 被拒: {result.Reason}");
                    return;
                }
                ...
                {
                    await PresentOpeningAsync(plan, linkedCts.Token);
                    FieldTraceHelper.RecordOccupancySnapshot("walkSandboxAfter", FlowTraceBatchTags.StartNode);
                }
                finally
                {
                }

                Cards?.AuditRegistryIntegrity("WalkSandbox.Settled");
                RefreshPersistentInBattleUi(animate: false);
                PlayerInfoHudPresenter.TryGetInstance()?.SyncFromCore(animate: false);
                Debug.Log(
                    $"[BattleSession] 跳格沙盒就绪 phase={phase.CurrentPhase}"
                    + $" avatar@{arch.GetModel<BoardModel>().AvatarSlot.Value}");
            }
            finally
            {
                _isBusy = false;
