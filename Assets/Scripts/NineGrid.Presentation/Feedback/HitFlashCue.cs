...
namespace NineGrid.Presentation.Feedback
{
    ...
    [MovedFrom(true, "NineGrid.Presentation.Performance", null, "CardHitFlashPerformance")]
    public sealed class HitFlashCue : MonoBehaviour, ILocalCue
    {
        ...
        {
            if (invocation.Target != null && invocation.Target != transform)
            {
                return;
            }
        ...
        private void EnsureFlashMaterialsInstalled()
        {
            ...
            {
                Debug.LogWarning(
                    $"{nameof(HitFlashCue)} on '{name}' could not resolve flash material.",
                    this);
                return;
            }
