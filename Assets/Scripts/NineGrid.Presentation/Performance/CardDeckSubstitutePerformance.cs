...
namespace NineGrid.Presentation.Performance
{
    /// <summary>
    /// 牌堆补牌：单张卡从牌堆原点飞入场地空槽。
    /// 手感对齐 <see cref="CardDeckEntryPerformance"/>（duration 0.3，OutQuint）。
    /// </summary>
    [DisallowMultipleComponent]
    public sealed class CardDeckSubstitutePerformance : MonoBehaviour
    {
        [Header("Deck Origin")]
        ...
        public void PlayPreview()
        {
            ...
            if (slots.Count == 0)
            {
                Debug.LogWarning($"[{nameof(CardDeckSubstitutePerformance)}] no slot anchors resolved.", this);
                return;
            }
