...
| `PlaytestFeedbackPanel` | `Ui/PlaytestFeedbackPanel.cs` | 局内试玩反馈：点子 Logo 打开 `输入框窗口`（默认失活，WorldSpace Canvas）；上勾提交 Bug、下勾提交意见，均在 FNS `游戏开发项目/九宫格登神/游戏开发协作/` **各新建一篇笔记**（不续写、不传 log）；成功后覆盖「提交意见按钮」下 TMP（Bug：`bug已提交，立刻加班修复！` / 意见：`建议收到啦，感谢你的支持！`）；叉关窗。凭据只读 gitignore 的 `StreamingAssets/fns-playtest.secret.json`；提交协程挂 DDOL 宿主，关菜单不中断上报 |
