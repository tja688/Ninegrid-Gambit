...
| `MainMenuLoadPanel` | `Ui/MainMenuLoadPanel.cs` | 主菜单「继续游戏」加载弹窗（`MainPanel/加载的UI槽位`，与 ContinueGame 按钮解挂、1x 缩放，默认失活）：列出已有存档（自动档优先 + 手动槽 ≤4，复用场景预置 4 行模板、多余行隐藏），打开时 `BattleUiDimmerOverlay.TryAcquire`；点击条目即 `RunSaveService.RequestLoadSlot`；`关闭面板 (1)`/Esc 关闭；无任何存档 `RequestOpen` 返回 false（`GameFlowController` 灰显按钮 + 拒绝音 + 主菜单 InfoNotice「暂无可用存档」）；弹窗底板吞点击防误触其下主菜单按钮 |
...
- **主菜单继续游戏**：`GameFlowController` 轮询 `ContinueGame` 按钮（hover/press/reject 三音）；无档时灰显并只播拒绝音 + InfoNotice，有档才 `MainMenuLoadPanel.RequestOpen()`（半黑屏 + 1x overlay）。弹窗/选人界面/纯黑屏/半黑屏任一激活时主菜单按钮轮询整体让位（Update 早退）。主菜单相位 `UIInfoHoverRouter` 抑制血条等局内 HUD 介绍。读档恢复失败（`TryBootstrapRestoredRun` / 收口超时）回主菜单并 InfoNotice「读档失败，已返回主菜单」。
