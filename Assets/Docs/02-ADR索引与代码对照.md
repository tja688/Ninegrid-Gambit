...
| [0047](../../docs/adr/0047-pipeline-fault-containment.md) 失控反应链熔断遏制 | 管线深度（64）/单 run 总量（4096）超限从抛异常改为熔断：丢弃超限分支、落 `PipelineFaultContained` 诊断事件+`Evt_PipelineFaultContained` QF 事件、命令原子收尾 | `C/Systems/ActionPipelineSystem.cs`（`ContainPipelineFault`）；表现侧 `P/Flow/BattleSession/BattleSessionExecutor.OnPipelineFaultContained`（LogError）；回归 `FlameIntenseBurningLoopRegressionTests` | 任何命令执行后批次照常打开、EventLog 结构完整（即使内容死循环）；熔断=待修内容 bug；防环根治在 DSL cause 标记（卡装配效果 Cause 缺省=挂载卡 DefId，精确排除必须显式配 `cause`） |
...
- **2026-08-12 当日新增**：0044（结算窗口位移挂起）、0045（卡面投影统一对账缝）、0046（本地化外挂翻译表）、0047（失控反应链熔断遏制）——四条均 accepted 且已随本清单核对落地代码。
