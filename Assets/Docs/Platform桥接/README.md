...
| `SteamPlatformBootstrap` | `NineGrid.SteamBridge/SteamPlatformBootstrap.cs` | `BeforeSceneLoad` 自举（2026-08-12 修订，ADR-0043）：Editor 下先查本机 EditorPrefs 开关（`NineGrid.Steam.EditorEnabled`，默认 false，未开完全不碰 SteamAPI）→ Development Build 或占位 AppId 的 Release 直发包整体跳过（临时措施，未上架前不触发 `RestartAppIfNecessary` 秒退）→ `Packsize/DllCheck` 校验 → Editor 下 `SteamAPI.IsSteamRunning()` 确认客户端在运行（只读状态不拉起）→ `RestartAppIfNecessary`（仅正式 AppId 的 Release Build）→ `SteamAPI.Init` → 挂回调泵 → 注册成就/平台信息后端 → 用云装饰器包住既有存档后端并 `SyncOnBoot`；失败即静默降级；`ShutdownFromPump` 还原全部 Hook + `SteamAPI.Shutdown` |
