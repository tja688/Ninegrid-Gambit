...
>
> **权威性声明**：本库以**当下代码实际实现**为准写成（文档/注释与代码冲突处均按代码事实记录并标注）。改代码时须同步更新对应区域文档（维护义务见 [`.cursor/rules/code-map-maintenance.mdc`](../../.cursor/rules/code-map-maintenance.mdc)）。边界与不变量：`docs/code-map/` + `docs/adr/` + `CONTEXT.md`；本库管全量代码事实与事故导航。
