...
- **Core 终态**：新增 `GamePhase.Victory / Defeat`。化身受到伤害后若最终血量仍为 0，会在 `OnFatalDamage` 等反应结算后进入 `Defeat`，避免抢在免死/复活效果之前判负。
- **楼层 / 节点推进**：`RunModel` 落地 `3 层 × 9 节点` 规则；`AdvanceNodeAction` 会在第 3 层第 9 节点尾段切到 `Victory`。旧的普通节点尾段仍保持 `NodeCompleted -> StartNode`。
- **Headless 单局模拟器**：在 `Assets/Scripts/NineGrid.Core.Tests/Simulation/` 新增：
  - `HeadlessCampaignDriver`：战役循环 driver，真实走 `StartNode/Attack/PickupItem/ClickEmpty/UseItem/SelectReward/SkipHelpChoice/SelectRoom/EnterRoom` command 通道。
  - `RandomAgent`：只从合法相邻交互、合法奖励/房间选择、未使用过的道具中随机选具体指令；策略 RNG 与 Core 内容 RNG 分离。
  - `RunTranscriptWriter`：按层/节点/指令聚合 `EventLog -> ActionLogRow`，输出人类可读战报到 `Logs/sim/`。
  - `HeadlessInvariantChecker`：首批低误报红灯，包括伤害/剩余血甲非负、扣减不超过伤害、金币流水对账。
  - `HeadlessBatchRunner`：单进程顺序跑 N 个 seed，导出 CSV/JSON 指标。
- **实验钩子**：
  - `HeadlessSimulationOptions.CatalogOverride` 支持注入调参后的 catalog。
  - `ForcedNodeCard` 支持把指定卡强制加入某层某节点的玩家/敌方牌池，用于受控实验。

### 8.2 使用入口

- 单局：
  - `new HeadlessCampaignDriver().Run(new HeadlessSimulationOptions { ... })`
  - 默认使用 `TableNineContentCatalog.CreateDefault()`；如需快速验 27 节点流程，可传入测试里的 smoke catalog 思路；如需真实内容压测，直接使用默认 catalog。
- 批量：
  - `new HeadlessBatchRunner().Run(new HeadlessBatchOptions { Count = N, ... })`
  - 输出：`Logs/sim/sim_batch_<startSeed>_<count>.csv/json`
- 战报：
  - `HeadlessSimulationOptions.WriteTranscript = true`
  - 输出：`Logs/sim/sim_<contentSeed>_<agentSeed>_<outcome>.txt`

### 8.3 验证结果

- Unity 编译通过。
- 新增测试 `P8HeadlessSimulationTests` 覆盖：
  - 化身致死后进入 `Defeat` 并锁指令；
  - 第 3 层第 9 节点尾段进入 `Victory`；
  - headless driver 能跑完 27 节点并写出战报；
  - batch runner 能顺序导出 CSV/JSON。
- 全量 `NineGrid.Core.Tests`：`128/128 Passed`。

### 8.4 已知边界

- 本轮**未做**多线程、多进程分片、MCTS / expectimax / 状态克隆。
- `RandomAgent` 是合法随机基线，不是“会玩”的平衡 AI；默认大内容下已经能真实跑出暴毙战报，说明死亡终态和战报链路可用，但通关率不能代表玩家强度。
- `UseItemCommand` 目前沿用现有 Core 行为：使用道具只发 `UseItemAction` 事件并触发效果，未在 PhaseSystem 层统一消费道具。Agent 侧用“同一 uid 只用一次”避免随机压测时重复刷同一张道具。
- 不变量检查先落低误报版本；完整伤害公式复算、奖励经济更细校验可继续往 `HeadlessInvariantChecker` 加。

### 8.5 下一步建议

1. 做 `GreedyAgent`：优先击杀低血威胁、优先拿治疗/直伤、避免无意义旋转，用于“会玩一点”的平衡基线。
2. 给 `RunTranscriptWriter` 增加摘要视图：每节点一行汇总 + 可选展开 Action 明细，便于大批量人工扫读。
3. 扩展 `HeadlessInvariantChecker`：复算玩家/怪物攻击、护甲/金币抵消、击杀奖励与房间收益。
4. 如果要做最优解或并行压测，再立项状态快照/回滚；这与 MCTS、多实例并行是同一底层改造。
