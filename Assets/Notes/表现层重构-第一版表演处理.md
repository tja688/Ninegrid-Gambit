...
| **PR3** | 战斗/牌组/棋盘 Flow；`HitFlashCue`/`CardShakeCue`/`DamageNumbersReaction`；`BoardCardHoverPresenter` | `CardAttackFlow` 仍可调 `HitFlashCue` | ✅ 已完成 |
| **PR4** | 拆 `ItemCardInteract`；迁 Projection 占位；删 `Performance/`、`Tools/`、`DrumRollDigitDisplay`；MCP 清场景 | `NineGrid.Core.Tests` 全绿；Unity Console 0 Error | ✅ 已完成（1 项既有内容测试失败） |

---

## PR3 落地记录（2026-06-30）

已迁入五池（原 `Performance/` 保留至 PR4 删除）：

| 池 | 新类 |
|----|------|
| `Flow/Battle/` | `CardAttackFlow`、`CardKillFlow`、`CounterattackFlow`、`CounterattackKillFlow` |
| `Flow/Board/` | `BoardRotateFlow` |
| `Flow/Deck/` | `CardDeckEntryFlow`、`CardDeckDealFlow`、`CardDeckSubstituteFlow` |
| `Feedback/` | `HitFlashCue`、`CardShakeCue`、`AttackTrailCue`（空壳） |
| `Reactions/` | `DamageNumbersReaction` |
| `Interaction/` | `BoardCardHoverPresenter` |

PreviewTool 已改引新 Flow 类型；`CardAttackFlow` 内部受击闪白改调 `HitFlashCue`。

**下步 PR4**：拆 `ItemCardInteract`、迁 Projection、删 `Performance/` + `Tools/`、MCP 清场景。

---

## PR4 落地记录（2026-06-30）

### ItemCardInteract 拆分（Interaction 池）

| 新类 | 职责 |
|------|------|
| `HandLayoutPresenter` | 演员基线、扇形 `Relayout` 收敛（`[MovedFrom ItemCardInteractPerformance]`） |
| `HandCardDragPresenter` | Hover 聚焦、拖拽跟手、区域着色 |
| `HandCardReturnPresenter` | 回手 / 确认打出 |
| `CardShakeCue` | 拒绝抖动（已有，DragPresenter 可选引用） |

`Shared/HandCardLayoutSolver` + `HandCardLayoutTarget` 从 `Performance.Layout` 迁入。

### 其余迁移

| 池 | 新类 |
|----|------|
| `Flow/Item/` | `ItemUseFlow` |
| `Reactions/` | `CardAcquisitionFlow`（委托 HandLayout + HandReturn）、`ModifierApplyReaction`、`EffectTriggerReaction` |
| `Projection/` | `InGameStatusProjection` |

### 删除

- 整个 `Performance/`（28 类 LEGACY）
- 整个 `Tools/`（5 个 PreviewTool）
- `Visuals/DrumRollDigitDisplay.cs`（`TableNineCardStatusView` 已仅用 `DirectDigitDisplay`）

### 验收

- Unity 编译：0 Error
- MainScene：无 PreviewTool / 旧 Performance 组件挂载（MCP 检索为空）
- `NineGrid.Core.Tests` EditMode：152 项中 1 项失败 — `P6ContentVisualTests.ResolverFallsBackMonsterFaceToDeckConvention`（内容路径，与本轮无关）
