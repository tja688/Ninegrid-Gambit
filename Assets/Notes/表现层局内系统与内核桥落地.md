...
- HUD（金币/互动数/遗物技能/阶段）走 **QF BindableProperty 直连**，接受结算瞬间即时更新。
- 卡面 HP/护甲**留在 Batch 时间线**（由战斗 Flow 内化驱动，批末 Reconcile 兜底对齐）。
...
## 已知缺口 / 后续跟进（非阻断）
- `UseItemCommand` 仅传 `itemUid`，目标/选项型帮助卡需先扩 Core Command 参数（`selectedCardUids/selectedOption`），本轮先做无目标道具。
- 道具选项覆盖层（`stat_boost` 三选一）Core 无 `PendingItemChoice`，暂保留表现层本地驱动，后续再下沉内核。
...
3. **HUD 阶段直连描述与实现不一致。** `RunModel.Phase` 的生产订阅在 `GamePhaseFlowShellProjection`，不是 `TableNineHudModelBinder`；状态面板 HP/护甲仍作为 `StatusPanelReconcilable` 进入批末 Reconcile。建议文档和蓝图统一表述为“HUD 资源数/内容条直连，阶段驱动 Shell/HUD 显隐，战斗数值仍由 Flow + Reconcile 兜底”。
4. **PlayMode 端到端测试缺失。** 目前 EditMode 44/44 通过，但 PlayMode 发现 0 个测试。若要声明生产闭环完成，需要补一个最小场景测试：加载 `MainScene`，等待 Bootstrap ready，触发一次合法 Attack/Pickup/ClickEmpty，断言 Batch opened、输入锁升降、Flow/Reconcile 执行、`PresentationFinishedCommand` 解锁。
...
| **2** | `UseItem` 选项/目标型道具 → SelectionFsm 闭环 | 待做 |
| **3** | PlayMode 端到端测试、蓝图 canvas 清理、文档终稿 | 待做 |
...
**批次 3 预告**
- 最小 PlayMode：`Bootstrap ready` → `Attack` → Batch → `PresentationFinished` → 输入解锁。
- 蓝图 canvas 删除 `Reaction 残留/调试区` 节点与 `PlannedReaction` 边；HUD 表述统一为「资源条直连 + 阶段驱动 Shell + 战斗数值 Flow/Reconcile」。