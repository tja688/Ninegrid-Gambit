...
>
> **2026-08-16 修订**：§5 的候选状态已部分过时——`ResetCurrentArmorAction` 现在会发 `ArmorChanged`，且 `PhaseSystem.StartNode` 已把重置放在 `FillEmptySlots`/`AvatarAppeared` 之前（A 已修）；`CardFaceStatHandler.ApplyHp/ApplyArmor` 已补 resolve/view miss 日志（B 已有诊断）；`BattleBeatScheduler` 已补开批丢弃与未消费诊断（C 已有诊断）。另新增一类语义断点：金币盔甲（`relic.gold_armor`）代偿甲伤时，`DamageDealt.ArmorDamage` 仍报“代偿前毛甲伤”，卡面甲却忠实跟随“净当前甲”，造成“卡面甲只增不减”的误报。完整复盘与修复计划见 [卡面护甲只增不减-金币盔甲吸收口径排查与修复计划-2026-08-16](卡面护甲只增不减-金币盔甲吸收口径排查与修复计划-2026-08-16.md)。
...
### 0.1 权威真相的位置（QF Model 实体）

**「权威」全部住在 QF 架构的 Model 里；卡面显示链刻意不经过 Model。**

- 架构单例：`NineGridArchitecture`（`NineGrid.Core/Architecture/NineGridArchitecture.cs:8`），`Configure()` 注册 8 个 QF Model（`NineGridArchitecture.cs:16-23`）。
- 卡实例真相：`CardRegistry`（QF Model，`Core/Models/CardRegistry.cs`）持有全部 `CardInstance`（**普通 C# 类，不是 QF Model**；`CardInstance.cs:7`）。每张卡的 `Stats`（`StatBlock`：BaseValues 基础值 + Modifiers 修饰器）、`Counters`、`Zone`、`Slot`、`FaceUp`、`Kind`、节奏字段都在这里。
- 有效值不是存的：`IStatSystem.GetEffectiveInt`（QF **System**）实时计算「基础 + 修饰器管线」→ 有效攻/甲/血。伤害结算与卡面显示都消费它。
- 其余真相：`BoardModel`（占格/盘面）、`DeckModel`（抽牌堆）、`PlayerModel`（金币）、`RunModel`（跑图进度）。
- 影子真相：`IActionPipelineSystem.EventLog`（QF **System** 上的事件日志）——全部数值变更的可重放历史，开局 Bootstrap 与手牌重 Spawn 用它还原卡面。

| 数字 | 权威位置 | 类型 |
|---|---|---|
| 某张卡存在、uid/defId/Kind | `CardRegistry` | QF **Model** |
| 基础攻/血/甲、**当前护甲**（`CurrentArmor` base） | `CardInstance.Stats`（StatBlock） | Model 内字段 |
| 修饰器（遗物/光环/条件） | `CardInstance.Stats.Modifiers` | Model 内字段 |
| 有效攻/甲/血（结算与卡面共同消费） | `IStatSystem.GetEffectiveInt` | QF **System**（算法） |
| 占格 / 抽牌堆 / 金币 / 跑图 | `BoardModel` / `DeckModel` / `PlayerModel` / `RunModel` | QF **Model** |
| 全部变更历史（重放用） | `IActionPipelineSystem.EventLog` | QF **System** |

**为什么显示链上看不到 Model**：ADR-0005 刻意切断「卡面 ↔ Model」的直读——指令只搬运结算后绝对值。Model 在链上只出现三处：① Core 动作写侧（`context.GetModel<CardRegistry>()` 改值 + 发射事件）；② 处理器解析侧（`CardFaceStatHandler.TryResolveCard` 经 `CardEntityLifecycleHook` 用 uid 找**表现层** `ManagedCard`——与 Core `CardInstance` 靠 uid 对号，两个世界不混用）；③ 开局重放侧（读 EventLog）。除此之外，卡面数字每次跳动都不经过 Model。
...
```mermaid
flowchart TD
    subgraph L1["L1 数值模型层（QF Model 权威）"]
        REG["CardRegistry（QF Model）<br/>CardInstance × N：<br/>Stats(StatBlock 基础值+修饰器) /<br/>Counters / Zone / Slot / FaceUp"]
        CA["StatArmorUtility<br/>三层护甲：基础 / 有效 / 当前"]
        RULE["IStatSystem（QF System）<br/>有效攻 = 基础+修饰器<br/>有效甲 = 基础+修饰器"]
        OTHER["BoardModel / DeckModel /<br/>PlayerModel / RunModel"]
    end
...
|---|---|---|---|---|
| **A** | **发射缺口：改值路径不发事件** | **已修**（2026-08-16 复核） | 卡面停在旧值 | `ResetCurrentArmorAction` 现在会发 `ArmorChanged`（PhaseActions.cs:68-96），且 `PhaseSystem.StartNode` 已把重置放在 `FillEmptySlots` 之前（PhaseSystem.cs:176-178）。开局「重置前值」疑点已不存在 |
| **B** | **认领即丢：Handler 认领了但没写入** | **已有诊断**（2026-08-16 复核） | 卡面停在旧值，现会 LogWarning | `CardFaceStatHandler` 对 `UpdateArmor` 恒返回 true（CardFaceStatHandler.cs:28-30,75-83）；但 `ApplyHp`/`ApplyArmor` 的 resolve miss 与 view miss 已补 `LogResolveMiss`/`LogViewMiss`（CardFaceStatHandler.cs:65-104,258-276），不再静默。怀疑 B 时先查 ConsoleLog |
| **C** | **批次吞没：打开的批从未被 Present 消费** | **已有诊断**（2026-08-16 复核） | 指令静默消失，现会 LogError | `OnBatchOpened` 仍先清空上一批 pending，但清空前已调用 `DiagnoseDiscardedPending`（BattleBeatScheduler.cs:32-39,369-397）；`DiagnoseUnconsumed` 仍只抓「已装载未消费」。怀疑 C 时先查 ConsoleLog 的 `[BattleBeatScheduler]` Error |
...
| **J** | **护甲 0 时事件不发** | 设计内 | 无影响 | `armorLoss == 0`（甲为 0 被打、金甲全抵）不发 `ArmorChanged`——甲不变所以不发是对的；但「甲被打到 0」的那次是 `armorLoss>0`，**会发**，不在此列。金甲全抵的观感问题见新增的 **K** |
| **K** | **金币盔甲：毛甲伤 vs 净当前甲** | **已修**（2026-08-16 复盘落地） | 卡面甲“只增不减”的观感误报 | `DamageDealt.ArmorDamage` 是金币代偿前毛甲伤（`CoreGameEvent.cs` 注释明说含金甲代偿），`DamageFloaterBeatHandler` 按它飘甲伤；而卡面甲跟随 `armorLoss`（净当前甲）。`relic.gold_armor` 全代偿时不发 `ArmorChanged` 是正确的。**修复后**：事件新增 `GoldAbsorbedArmor` 拆账字段；飘字改飘「净甲伤（绿灰）+ 金币 -N（金色）」，BattleLog 同样拆账，不再把代偿部分报成甲伤。详见 2026-08-16 复盘笔记 |
...
**若症状出现前刚获得 `relic.gold_armor`，优先按 K 核对**：看 `DamageDealt` 是否有 `GoldModified(message=goldArmor)` 同批；有则卡面甲不动是当前甲未变，不是断链。
...
| 层 | 文件 |
|---|---|
| Core Model 注册 | `NineGrid.Core/Architecture/NineGridArchitecture.cs` |
| Core 数值模型 | `NineGrid.Core/Models/CardRegistry.cs`、`CardInstance.cs`（StatBlock/Counters）、`Domain/Stats/StatArmorUtility.cs`、`StatPipeline.cs`、`StatModifier.cs` |
