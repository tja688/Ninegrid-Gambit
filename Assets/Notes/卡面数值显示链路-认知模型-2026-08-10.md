...
| **K** | **金币盔甲：毛甲伤 vs 净当前甲** | 语义断点 | 卡面甲“只增不减”的观感误报 | `DamageDealt.ArmorDamage` 是金币代偿前毛甲伤（`CoreGameEvent.cs` 注释明说含金甲代偿），`DamageFloaterBeatHandler` 按它飘甲伤；而卡面甲跟随 `armorLoss`（净当前甲）。`relic.gold_armor` 全代偿时不发 `ArmorChanged` 是正确的，但飘字仍报甲伤，造成“卡面漏扣”。详见 2026-08-16 复盘笔记 |
