...
todos:
  ...
  - id: visual-resolver
    ...
    status: completed
  - id: visual-editor
    content: Unity Editor 配图工具（icon/face visual_id + Card Frame 配色 Tab + visual_asset 回写）
    status: completed
  - id: ui-wire
...
## Phase A 图片资产化（2026-06 已落地）

在 Phase 1/2 基础上，将「物理路径 key」升级为「业务 visual_id + 注册表」，并将 Card Frame 改为稀有度配色。

### 新增 Luban 表

| 表 | xlsx | 用途 |
|:--|:--|:--|
| `TbVisualAsset` | `visual_asset.xlsx` | `visual_id` → `asset_key`（逻辑加载地址，非 `Assets/...` 路径） |
| `TbCardFrameStyle` | `card_frame_style.xlsx` | `frame.white` / `frame.gold` / `frame.boss` … → RGBA |

`content_visual.xlsx` 仍存 `icon_key` / `face_key`，但语义为 **visual_id**（如 `visual.icon.help.bomb`）。`frame_key` **已废弃**（卡框不再逐卡配图）。

### 运行时链路

```
content_id
  → ContentVisualResolver（join 内核 rarity / elite / boss）
  → icon/face: visual_id + convention asset_key → VisualAssetCatalog → ResourcesVisualLoader
  → frame: frame_style_id → CardFrameStyleCatalog → SpriteRenderer.color
```

核心代码：`VisualIdNaming` · `VisualAssetKeyNaming` · `IContentVisualLoader` · `TableNineCardVisualBinder`（frame 只设色不换图）。

### Editor 工作流

1. **内容配图** Tab：拖 Sprite → 写 `visual.{slot}.{content_id}` + upsert `visual_asset.xlsx`
2. **Card Frame 配色** Tab：`ColorField` → PATCH `card_frame_style.xlsx`
3. 菜单：`Migrate Legacy Path Keys` · `Clear Deprecated frame_key Column` · `Regenerate Luban`

### 后续演进（未做）

- Phase B：`visual_set_id` / 动画 `kind=frame_animation`
- Phase C：Addressables + Sprite Atlas 分组
