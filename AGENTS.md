...
- **战斗 / 流程 / 表现日志分析**：Play 结束导出至 `Assets/Notes/Logs/`（`CoreLog` / `PerfLog` / `OtherLog/RegistryLog` / `OtherLog/BattleLog`）；分析见 `.cursor/skills/table-nine-battlelog-analysis/`。日志目录已纳入工作区索引；分析前先 Glob 最新 `corelog-*`，QuickTest 文件名含 `-QuickTest-`。
